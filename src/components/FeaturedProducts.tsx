          ...
          <Card key={product.id} className="border rounded-2xl shadow-sm hover:shadow-md transition group">
            <CardContent className="p-4">
              <div className="relative mb-4">
                ...
                >
                  <Heart className={`h-4 w-4 ${isInWishlist(product.id) ? 'fill-amber-600 text-amber-600' : 'text-gray-600'}`} />
                </Button>
              </div>
              <h4 className="text-lg font-medium mb-1">{product.name}</h4>
              <p className="text-sm text-gray-500 mb-2">{product.description}</p>
              <p className="text-amber-600 font-semibold text-lg mb-4">{product.price}</p>
